...
- [x] **Phase 44: Bug Fixes & 18.0 Template Parity** - Fix env.ref() double prefix, audit model record_rule_scopes, 18.0 groups= blocks, and port 18.0 audit trail (gap closure) (completed 2026-03-07)
...
### Phase 44: Bug Fixes & 18.0 Template Parity
**Goal**: Fix 3 runtime bugs found by milestone audit and achieve 18.0 template parity for groups= and audit trail blocks
**Depends on**: Phase 43 (integration tests must exist to verify fixes)
**Requirements**: BIZL-02, SECR-01, SECR-02, SECR-03 (gap closure — requirements already satisfied, fixing runtime bugs)
**Gap Closure**: Closes all integration/flow/tech-debt gaps from v3.2 audit
**Success Criteria** (what must be TRUE):
  1. Notification `env.ref()` calls produce single module prefix (`module.xml_id`, not `module.module.xml_id`) in both 17.0 and 18.0 templates
  2. `audit.trail.log` synthesized model includes `record_rule_scopes: []` so `record_rules.xml.j2` renders without StrictUndefined crash when audit+approval combined
  3. 18.0 `model.py.j2` contains all 6 `groups=` field attribute blocks matching 17.0 parity
  4. 18.0 `model.py.j2` contains audit trail blocks (has_audit, _audit_skip, audit log creation) matching 17.0 parity
  5. Kitchen sink integration test generates all 16+ files (not just 10) with audit+approval+notifications+webhooks combined
**Plans**: 1 plan

Plans:
- [ ] 44-01-PLAN.md — Fix 3 bugs + port 18.0 audit/groups blocks + update integration tests
...
| 44. Bug Fixes & 18.0 Parity | 1/1 | Complete   | 2026-03-07 | - |
