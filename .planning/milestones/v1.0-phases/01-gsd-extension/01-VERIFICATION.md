...
|---|-------|--------|----------|
| 1 | User can clone odoo-gen into `~/.claude/odoo-gen/` and run a setup command that registers all commands | VERIFIED | `install.sh` exists with `set -euo pipefail`, checks GSD at `~/.claude/odoo-gsd`, checks uv, creates `.venv`, runs `uv pip install`, copies `commands/*.md` to `~/.claude/commands/odoo-gen/`, symlinks `agents/*.md` into `~/.claude/agents/`, writes manifest, verifies via `--version` |
